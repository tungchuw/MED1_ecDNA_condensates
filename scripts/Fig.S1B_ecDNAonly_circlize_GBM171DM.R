...
#source('/home/tjongh/myBin/myR/hcl_colors.R')
...
circosplot_EC_gene <- function(col.se='#9834eb', col.chip3='#f40000', col.chip2='#3acae0', col.chip1='#e0a33a', col.score.trans='#04cc65', col.score.cis='black', cex.se=0.7, geneplot=FALSE, maj.tick=1e9){
    ...
    #H3K27Ac Chipseq
    circos.genomicTrackPlotRegion(bwsig.df2[,c(1:4)], ylim = c(0, 9.2), stack = FALSE, panel.fun = function(region, value, ...) {
    ...
    if(ncol(bwsig.df2)>4 & geneplot==FALSE){
        circos.genomicTrackPlotRegion(bwsig.df2[,c(1:3,5)], ylim = c(0, 9.2), stack = FALSE, panel.fun = function(region, value, ...) {
    ...
    #MED1 Chipseq
    circos.genomicTrackPlotRegion(bwsig.df[,c(1:4)], ylim = c(0, 10), stack = FALSE, panel.fun = function(region, value, ...) {
    ...
    if(ncol(bwsig.df)>4 & geneplot==FALSE){
        circos.genomicTrackPlotRegion(bwsig.df[,c(1:3,5)], ylim = c(0, 10), stack = FALSE, panel.fun = function(region, value, ...) {
            circos.genomicLines(region, value, area =TRUE, type='l', lwd=2, border=NA, col = col.chip1, ...)
        },  track.height = 0.12,  bg.border = NA)
    }
    ...
    # col.score.trans='#04cc65', col.score.cis='black'
    ...
    # },  track.height = 0.12,  bg.border = bordcol)
    # write.table(ec.mergeFrag , file=paste0(RUN, '.ec.mergeFrag.nGEMs.circos.tsv'), col.names=F, row.names=F, quote=F, sep="\t")

    circos.genomicTrackPlotRegion(nodeScore.cis, ylim = c(0, 20), stack = FALSE, panel.fun = function(region, value, ...) {
    ...
    circos.genomicTrackPlotRegion(nodeScore.trans, ylim = c(0, 25), stack = FALSE, panel.fun = function(region, value, ...) {
...
#cut -f1-7 /projects/wei-lab/proj-SuperEnhancer/ChIA-PIPE/PC3NCI60/PC3NCI60.DA.itx |bedtools pairtobed -type both  -a stdin -b /projects/wei-lab/proj-SuperEnhancer/regions/PC3DM.ecDNA.bed |cut -f1-7 | sort -k1,1 -k2,2n |uniq
...
#itxFile <- paste0(chiapetDir, run2names[RUN],'/',run2names[RUN], '.DA.itx')
itxFile <- "/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/ChIA-PIPE/HDtreat/ACP0020/ACP0020.DA.itx"
...
#libdir=paste0('/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/ChIA-PIPE/',run2names[RUN],'/')
libdir="/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/ChIA-PIPE/HDtreat/ACP0020/"

#message(run2names[RUN], " chiadrop library being plotted: ", RUN )

#peakfile=paste0(libdir, run2names[RUN], ".Pol2_chipseq.narrowPeak")
peakfile="/net/nwgc/vol1/nobackup/nocleanup/tungch/A0095/peak/A0095.all_peaks.narrowPeak"
...
chipdir='/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/1x51_chipseq_processed/A0041/'
med1.bwfiles = medlibs[[seNames[run2names[RUN]]]]
bw1=paste0(chipdir,"fe_", med1.bwfiles[1], ".bw")
bw2=paste0("/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/1x51_chipseq_processed/A0006/normalized/fe_A0006.bw")
bw1
bw2

ecdnaRegfile = "B171.ecDNA.bed"
roseBed = paste0(regiondir, seNames[run2names[RUN]] , '.SErose.bed')

#
genome='hg38'
inclCHR=paste0('chr',c(1:22,'X'))
#allcyto = readRDS('/net/gs/vol3/software/modules-sw/R/4.3.1/Linux/CentOS7/x86_64/bin/R/library/circlize/extdata/cytoband_list.rds') #this contains hg38 as well
...
system(bedcmd)
#cytoband.df = read.table(tmpout, colClasses = c("character", "numeric","numeric", "numeric", "character", "numeric","numeric","character", "character", "numeric"), sep = "\t");
...
tsvinput=paste0('/net/nwgc/vol1/sharing/Wei_Lab/USERS/tungch/tjongh/ec/test_getMergedFragment_annot.chiadrop/',RUN,'.mergedFrag_ecGEMs.annot.tsv')
mergeFrag = read.delim(tsvinput, stringsAsFactors=F)
message('Get merge fragements ', tsvinput)
...
load(paste0("/net/nwgc/vol1/sharing/Wei_Lab/USERS/tungch/tjongh/ec/test/anno_Rdata/",RUN,".annotChiaDrop.ecPol2.RData"))
message('Loading fragments from ', paste0("/net/nwgc/vol1/sharing/Wei_Lab/USERS/tungch/tjongh/ec/test/anno_Rdata/",RUN,".annotChiaDrop.ecPol2.RData"))
...
#ectrans.ec.bed.GEMid = paste(ectrans.ec.bed, transBC, sep='\t')
...
transpare = 0.7
# ipet.bar = c(round(min(ipet),1), round(min(ipet)+max(ipet)/2,1), round(max(ipet),1))
...
# pdf(paste0(RUN, '.', seNames[run2names[RUN]], ".ecCircos.",bplab,"_",med1.bwfiles[1],"_with_ChIAPET.chiapet_cis_trans_nGEMs.pdf"), height=8, width=8, pointsize=15);
# pdf(paste0(RUN, '.', seNames[run2names[RUN]], ".ecCircos.",bplab,"_",med1.bwfiles[1],"_with_ChIAPET.chiapet_cis_trans.pdf"), height=8, width=8, pointsize=15);
# pdf(paste0(RUN, '.', seNames[run2names[RUN]], ".ecCircos.",bplab,"_",med1.bwfiles[1],"_with_ChIAPET.chiapet_trans.pdf"), height=8, width=8, pointsize=15);
...
#circ1 <- circosplot_EC_gene(geneplot=F, maj.tick=1E10, col.chip1='#b00448')
#circ2 <- circosplot_EC_gene(geneplot=T, maj.tick=1E10, col.chip1='#b00448')
#ggsave(circ1,file = paste0(RUN, '.', seNames[run2names[RUN]], ".ecCircos.",bplab,"1.pdf"))
#ggsave(circ2,file = paste0(RUN, '.', seNames[run2names[RUN]], ".ecCircos.",bplab,"2.pdf"))
#
##Put legend
