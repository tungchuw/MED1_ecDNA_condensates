...
circosplot_EC_gene <- function(col.se='#9834eb', col.chip3='#f40000', col.chip2='#3acae0', col.chip1='#e0a33a', col.score.trans='#04cc65', col.score.cis='black', cex.se=0.7, geneplot=FALSE, maj.tick=1e9){
    ...
    circos.genomicTrackPlotRegion(nodeScore.trans, ylim = c(0, 265), stack = FALSE, panel.fun = function(region, value, ...) {
       circos.genomicLines(region, value, area =FALSE, border=NA,  type='h', lwd=1, col = col.score.trans, ...)
        circos.yaxis(side = "left", at = c(0, 263), labels = FALSE, col = NA)
    },  track.height = 0.12,  bg.border = bordcol)
...
ecdnaRegfile = paste0("B171.ecDNA.bed")
...
load(paste0('/net/nwgc/vol1/sharing/Wei_Lab/USERS/tungch/tjongh/ec/test/anno_Rdata_PC3_B171ecRegion/',RUN,'.annotChiaDrop.ecPol2.RData'))
message('Loading fragments from ', paste0('/net/nwgc/vol1/sharing/Wei_Lab/USERS/tungch/tjongh/ec/test/anno_Rdata_PC3_B171ecRegion/',RUN,'.annotChiaDrop.ecPol2.RData'))
