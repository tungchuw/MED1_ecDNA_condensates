...
woIntersect <- bedcmd("bedtools intersect -wo -a ")
intersectBed <- bedcmd("bedtools intersect -u -a ")
intersectPair <- bedcmd("bedtools pairtobed -type both -a ")
bedmerge <- bedcmd("bedtools merge  -c 4 -o sum ")
...
roseBed <- file.path(cell_dir, paste0(seNames[run2names[RUN]] , ".SErose.bed"))
geneSource <- file.path(base_dir, paste0('hg38.Genes.itxAnnotation.bed'))
fragdf <- readRDS(cell_dir, paste0("/chiadrop/PC3_chiadrop_fragdf.rds"))
