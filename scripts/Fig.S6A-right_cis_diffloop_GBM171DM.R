...
#collect all anchors from libraries & replicates
...
#Use DESeq2 for calling differential interactions
...
volca = ggplot(data=a, aes(x=log2FoldChange, y=-log10(padj), shape=group)) +
  geom_point(aes(col=group, size=1)) +
...
pdedf$group = factor(ifelse(pdedf$threshold == FALSE, "below threshold 30255",
			ifelse(pdedf$pair == "ec-ec", "ec 18", "non-ec 1371")))
