...
ecdnaRegfile = "COLO320DM.ecDNA.bed"
...
#Super Enhancers
bedcmd = paste(woIntersect, "COLO320DM.SErose.bed",' -b', tmpcyto,'>', tmpout)
system(bedcmd)
g = read.table(tmpout, stringsAsFactors=F)
...
# Scale bar
# transpare = 0.5
# # ipet.bar = c(round(min(ipet),2), round(min(ipet)+max(ipet)/2,2), round(max(ipet),2))
# # ipet.bar = c(round(min(ipet),2), round(max(ipet),2))
...
# # col_fun = colorRamp2(ipet.bar, c("#5A97BF", "#F26052"), transparency=0)
# # col_fun = colorRamp2(ipet.bar, c("#5A97BF","#B2AEF9", "#F26052"), transparency=0.7)
# # col_fun = colorRamp2(ipet.bar, c("#0433FF","#711C8D", "#FF2500"), transparency=0.9)
transpare = 0.7
