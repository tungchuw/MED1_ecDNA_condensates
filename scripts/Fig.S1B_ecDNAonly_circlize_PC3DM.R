...
circosplot_EC_gene <- function(col.se='#9834eb', col.chip3='#f40000', col.chip2='#3acae0', col.chip1='#e0a33a', col.score.trans='#04cc65', col.score.cis='black', cex.se=0.7, geneplot=FALSE, maj.tick=1e9){
    ...
    circos.genomicTrackPlotRegion(nodeScore.cis, ylim = c(0, 210), stack = FALSE, panel.fun = function(region, value, ...) {
    ...
    circos.genomicTrackPlotRegion(nodeScore.trans, ylim = c(0, 265), stack = FALSE, panel.fun = function(region, value, ...) {
...
itxFile <- "/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/ChIA-PIPE/HDtreat/ACP0010/ACP0010.DA.itx"
...
ecdnaRegfile = paste0(seNames[run2names[RUN]] ,".ecDNA.bed")
...
tsvinput=paste0("/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/chiadrop/",RUN,"/rs3/",RUN,".mergedFrag_ecGEMs.annot.tsv")
mergeFrag = read.delim(tsvinput, stringsAsFactors=F)
message('Get merge fragements ', tsvinput)
# tsvinput=paste0("/net/nwgc/vol1/sharing/Wei_Lab/proj-SuperEnhancer/chiadrop/",RUN,"/",RUN,".mergedFrag_ecGEMs.annot.tsv")
# mergeFrag = read.delim(tsvinput, stringsAsFactors=F)
# message('Get merge fragements ', tsvinput)
...
load(paste0(chiadropDir,RUN,'/rs3/',RUN,'.annotChiaDrop.ecPol2.RData'))
message('Loading fragments from ', paste0(chiadropDir,RUN,'/rs3/',RUN,'.annotChiaDrop.ecPol2.RData'))
